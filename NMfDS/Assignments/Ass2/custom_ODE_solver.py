...
def ODE_solver(func, x0, t_max, eval_pts, tol=1e-12, t_min=0, method='DOP853', hamiltonian=0):
    ...
    sol = sp.integrate.solve_ivp(
        func, [t_min, t_max], x0, method=method, t_eval=t_eval, atol=1e-16, rtol=1e-16)
    ...
    if hamiltonian == 0:
        ...
        for i in range(len(sol.t)):
            if abs(hamiltonian(sol.y[:, i]) - hamiltonian(x0)) > tol:
                ...
                break
    if len(x0) == 2:
        # Independent of the hamiltonian, check if the determinant of the matrix [[x3, x4], [x5, x6]] is constant
        for i in range(len(sol.t)):
            if abs(sol.y[2, i]*sol.y[5, i] - sol.y[3, i]*sol.y[4, i]-1) > tol:
                print("Determinant of the matrix is not constant for all points. Error = ", abs(
                    sol.y[2, i]*sol.y[5, i] - sol.y[3, i]*sol.y[4, i])-1)
                print("Determinant of the matrix at x0 = ",
                      sol.y[2, 0]*sol.y[5, 0] - sol.y[3, 0]*sol.y[4, 0])
                print("Determinant of the matrix at x = ",
                      sol.y[2, i]*sol.y[5, i] - sol.y[3, i]*sol.y[4, i])
                break
